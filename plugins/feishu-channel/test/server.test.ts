...
import { recordObservedBots } from '../src/observed-bots-store'
import {
  createChannelCore,
  loadCredentials,
  readEnvFile,
...
// ── available_bots injection ─────────────────────────────────────────────────

/** A group im event from an allowed sender, with optional chat_id. */
function rawGroupIm(chatId = 'oc_grp', mentionsBotSelf = false): Record<string, unknown> {
  return {
    sender: { sender_id: { open_id: 'ou_sender' }, sender_type: 'user' },
    message: {
      message_id: 'om_grp',
      ...
      chat_type: 'group',
      message_type: 'text',
      content: '{"text":"hey group"}',
      create_time: '1700000000000',
      mentions: mentionsBotSelf
...
describe('createChannelCore — available_bots injection', () => {
  test('injects <available_bots> block for group delivery with known peer bots', async () => {
    const transport = new FakeTransport('ou_self')
    saveAccess(accessFile, {
      dmPolicy: 'pairing',
      groupPolicy: 'allowlist',
      allowFrom: [],
      groups: { oc_grp: { requireMention: false, allowFrom: [] } },
      pending: {},
    })
    recordObservedBots(dir, transport.appId, 'oc_grp', [{ openId: 'ou_peer', name: 'BotPeer' }])

    const notes: Note[] = []
    const core = makeCore(transport, notes)
    await core.handleEvent(IM_MESSAGE_EVENT_TYPE, rawGroupIm())

    expect(notes).toHaveLength(1)
    expect(notes[0]?.content).toContain('<available_bots>')
    expect(notes[0]?.content).toContain('ou_peer')
    expect(notes[0]?.content).toContain('BotPeer')
  })

  test('does not inject block for p2p delivery', async () => {
    const transport = new FakeTransport('ou_self')
    saveAccess(accessFile, {
      dmPolicy: 'allowlist',
      groupPolicy: 'block',
    ...
    // seed a bot for an arbitrary chat — should NOT appear in p2p delivery
    recordObservedBots(dir, transport.appId, 'oc_grp', [{ openId: 'ou_peer', name: 'BotPeer' }])

    const notes: Note[] = []
    const core = makeCore(transport, notes)
    await core.handleEvent(IM_MESSAGE_EVENT_TYPE, rawIm('om_p2p', 'oc_chat'))

    expect(notes).toHaveLength(1)
    expect(notes[0]?.content).not.toContain('<available_bots>')
  })

  test('filters self from the injected bots block', async () => {
    const transport = new FakeTransport('ou_self')
    saveAccess(accessFile, {
      dmPolicy: 'pairing',
      groupPolicy: 'allowlist',
    ...
    // record self + peer
    recordObservedBots(dir, transport.appId, 'oc_grp', [
      { openId: 'ou_self', name: 'BotSelf' },
      { openId: 'ou_peer', name: 'BotPeer' },
    ])
    ...
    expect(content).not.toContain('ou_self')
    expect(content).toContain('ou_peer')
  })

  test('does not inject block when no external bots are known', async () => {
    const transport = new FakeTransport('ou_self')
    saveAccess(accessFile, {
      dmPolicy: 'pairing',
      groupPolicy: 'allowlist',
