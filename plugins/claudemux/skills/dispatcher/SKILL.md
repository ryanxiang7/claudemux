...
`<repo>` throughout this skill is a path relative to the dispatcher dir. Usually that is a direct sibling subdirectory (`my-repo`), but a nested worktree works too — pass the multi-segment path (`web-project/feature-x`) and `tm` handles it. `tm spawn my-repo` creates a tmux session `teammate-my-repo` with cwd `<dispatcher-dir>/my-repo` and runs `claude` inside it; for a nested `<repo>` the session name and `/tmp/teammate-*` files use a slug that folds every `/` to `-` (`teammate-web-project-feature-x`), while the cwd stays the real nested path. Pass the same `<repo>` value to every verb — `tm states` shows it back to you in that form.
