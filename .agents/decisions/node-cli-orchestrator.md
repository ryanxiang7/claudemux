# The 1.0 line retires the MCP-native core for a pure Node `tm` CLI
...
[Decision mcp-native-orchestration-core](/.agents/decisions/mcp-native-orchestration-core.md) chose
...
- **Completion-perception — the primary reason.** mcp-native-orchestration-core's own design (its §6)
...
- **The single resident process — the secondary reason.** A resident core is
  ...
  restart, and to reconcile after a crash. mcp-native-orchestration-core's whole registry
...
- **`tm` stays a CLI; it stops being Bash.** The native-TypeScript verb
  implementations written for Phase B of mcp-native-orchestration-core's migration (then in
...
- **Process supervision moves to claudemux.** A `codex app-server` is a
  ...
  protocol file set, distinct from mcp-native-orchestration-core's in-memory `registry.ts`, which is
...
- **[Decision mcp-native-orchestration-core](/.agents/decisions/mcp-native-orchestration-core.md)
  ...
  longer the plan. What mcp-native-orchestration-core's migration *built* is not wasted: its Phase B
...
- **`core/`'s MCP modules are removed** — `server.ts`, `socket-transport.ts`,
  ...
  roadmap in the spec.
- **The dispatcher adapter collapses to nothing.** mcp-native-orchestration-core needed a per-dispatcher
  ...
  line. mcp-native-orchestration-core's *model-agnostic dispatcher* goal is dropped with the core: a
...
- **Completion-awareness is the 0.x model, kept.** `tm`'s atomic verbs block on
  ...
  mcp-native-orchestration-core's design was forced back onto through its shim — so the pivot loses no
...
- **Process supervision is new work claudemux owns.** Under mcp-native-orchestration-core the resident
...
- **Enforcement.** The experimental-protocol hazard is guarded by the pinned
  ...
  behavior-preserving conformance harness from mcp-native-orchestration-core's Phase B is kept — the verb
...
- [decisions/mcp-native-orchestration-core.md](/.agents/decisions/mcp-native-orchestration-core.md) — the superseded decision; its Phase B native-verb work is carried forward.
